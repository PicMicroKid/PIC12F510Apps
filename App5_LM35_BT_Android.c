/*
 * File:   LM35_BT_Android.c compiler version XC8 v2.10
 * Author: MicroKid
 * Monitoring temperature with use of LM35 on the ANDROID 
 */ 
#include <xc.h>
...
void SETUP_initialize(void)
    ...
    TRISGPIO = 0b00001100;  //'1': for inputs; '0': for outputs
                            ...
                            //GP4 is set as ...
                            //GP3 is set as ... for
                            //GP2 is set as input for LM35 (ADC)
...
void main(void)
    ...
    for(;;)
        ...
        if (!ADCON0bits.GO)     //if GO bit is clear, ADC is done and ready to read
            {
            //POT_value = (ADRES-1)* 2 + 2;    //for 5 Vdc supply voltage
            //POT_value = (ADRES-1) + 4;    //for 3.3 Vdc supply voltage
            POT_value = ADRES + 8;    //for 3.3 Vdc supply voltage
